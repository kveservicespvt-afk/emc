...
- **OTP/SMS:** no SMS or WhatsApp Business API is wired up yet (per spec, deferred until a paid provider is chosen). The OTP is always logged server-side; set `SHOW_DEV_OTP=true` (server env) to also return it in the `/api/auth/otp/request` response and show a "DEV MODE" banner with the code on the login screen — deliberately independent of `NODE_ENV` so it can be switched on for a live demo deploy, not just local dev. **Set to `false`/unset before real customer launch**, since anyone could otherwise log in as any phone number without receiving a real SMS.
